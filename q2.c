...
int main(int argc, char **argv){
    // if correct no of arguments is not provided then throw an error and exit
    if(argc < 4){
        strl = sprintf(str,"Please Provide Correct Number of Arguments\n");
        write(1,str,strl);
        exit(0);
    }
    ...
    int totalparts = (int)(argv[2][0] - '0');
    int part = (int)(argv[3][0] - '0');

    // get file length and initialise some variables
    off_t fileLength = lseek(f_read,0,SEEK_END);
    off_t sizeofpart = fileLength/totalparts ;

    // get chunk size according to size of part to be processed
    long chunk_size = getchunksize(sizeofpart);

    // move pointer to end of part to be reversed
    lseek(f_read,part*sizeofpart,SEEK_SET);
    ...
    while (1){
        ...
        bytes_left = sizeofpart - bytes_read;
        if(bytes_left <= chunk_size){
            ...
            strl = sprintf(str,"\rProgress: %.2f%%",(float)bytes_read * 100 / sizeofpart);
            write(1,str,strl);
